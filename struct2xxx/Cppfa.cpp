...
Cppfa::Cppfa()
{
    ...
    {
        if (x.size() != 3) { return cppfa::Object(); }
        int x0 = x[0];
        double x1 = x[1];
        double x2 = x[2];
        return (x0) ? x1 : x2;
    ...
    register_function("abs", [](ObjectVector& x) -> Object
        {
            if (x.size() == 0) { return cppfa::Object(); }
            double x0 = x[0];
            return abs(x0);
        });
    ...
    register_function("pow", [](ObjectVector& x) -> Object
        {
            if (x.size() <= 1) { return cppfa::Object(); }
            double x0 = x[0];
            double x1 = x[1];
            return pow(x0, x1);
        });
...
void Cppfa::expand_comma(CalUnit& c1, std::vector<CalUnit>& v)
{
    if (c1.str == ",")
    {
        for (auto& c2 : c1.v)
...
CalUnit& Cppfa::find_right_side(CalUnit& c1)
{
    CalUnit* p = &c1;
    while (p->v.size() > 0)
    {
...
CalUnitType Cppfa::guess_char(char c)
{
...
//分割语法
std::list<CalUnit> Cppfa::split(std::string& str)
{
    std::string r;
    std::list<CalUnit> rv;
...
//表达式语法树
//参数含义：是否合并{}，是否合并[]，是否合并()
CalUnit Cppfa::combine_all_cal(std::list<CalUnit>& ppp, bool curly, bool square, bool round)
{
    //合并{}
...
//查找现有最内层括号，并返回位置
std::list<CalUnit>::iterator Cppfa::inside_bracket(std::list<CalUnit>& ppp, std::list<CalUnit>& ppp2, const std::string& bl, const std::string& br)
{
    bool have = false;
    auto it = ppp.begin();
    for (; it != ppp.end(); ++it)
...
void Cppfa::combine_curly_backet(std::list<CalUnit>& ppp)
{
    while (true)
    {
...
void Cppfa::combine_square_backet(std::list<CalUnit>& ppp)
{
    while (true)
    {
        std::list<CalUnit> ppp2;
...
void Cppfa::combine_round_backet(std::list<CalUnit>& ppp)
{
    while (true)
    {
        std::list<CalUnit> ppp2;
...
void Cppfa::combine_ops(std::list<CalUnit>& ppp)
{
...
void Cppfa::combine_semi(std::list<CalUnit>& ppp)
{
    for (auto it = ppp.begin(); it != ppp.end();)
    {
        if (it->can_cal())
...
void Cppfa::combine_keys(std::list<CalUnit>& ppp)
{
    //合并关键字，从右向左
    auto it = ppp.end();
    while (it != ppp.begin())
...
void Cppfa::combine_types(std::list<CalUnit>& ppp) {}

void Cppfa::register_function(const std::string& name, func_type func)
{
    functions[name] = func;
}

void Cppfa::register_user_data(const std::string& name, void* p)
{
    user_data[name] = p;
}

void Cppfa::register_parameter(const std::string& name, Object o)
{
    parameters[name] = o;
}

void* Cppfa::get_user_data(const std::string& name)
{
    return user_data[name];
}

Object Cppfa::run_function(const std::string& name, std::vector<CalUnit>& vc)
{
...
Object& Cppfa::get_parameter(CalUnit& c)
{
    return parameters[convert_parameter_name(c)];
}

std::string Cppfa::convert_parameter_name(CalUnit& c)
{
...
bool Cppfa::check_parameter(CalUnit& c)
{
    return parameters.count(convert_parameter_name(c));
}

void Cppfa::check_cal_unit(CalUnit& c, CalUnit* father)
{
